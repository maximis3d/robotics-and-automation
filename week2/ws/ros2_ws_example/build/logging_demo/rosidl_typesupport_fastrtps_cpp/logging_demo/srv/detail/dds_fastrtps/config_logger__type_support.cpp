...
#include "logging_demo/srv/detail/config_logger__rosidl_typesupport_fastrtps_cpp.hpp"
  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = logging_demo::srv::ConfigLogger_Request;
    is_plain =
      (
...
static size_t _ConfigLogger_Request__max_serialized_size(char & bounds_info)
{
  ...
  ret_val = max_serialized_size_ConfigLogger_Request(full_bounded, is_plain, 0);
  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    // We still need to check that the in-memory alignment
    // is the same as the CDR mandated alignment.
    using DataType = logging_demo::srv::ConfigLogger_Response;
    is_plain =
      (
...
static size_t _ConfigLogger_Response__max_serialized_size(char & bounds_info)
{
  ...
  ret_val = max_serialized_size_ConfigLogger_Response(full_bounded, is_plain, 0);
